var whatsappchat = {    
    configUser :function(options){
        var defaultSettings = {
            users : [''],
            headerMessage: 'Escríbenos para cualquier consulta!',
            chatBoxMessage: 'Nuestro equipo responderá en pocos minutos',
            color: '#4ee676',
            selector: '',
        };
        ...
        if(settings.selector != ''){
            ...
            settings.users.forEach(function (user, index){
                ...
                var image = 'assets/plugins/whatsApp/img/user.png';
                if(typeof settings.users[index].image != 'undefined' ){
                    image = settings.users[index].image;
                }
                depart = '';
                if(typeof settings.users[index].depart != 'undefined' ){
                    depart = settings.users[index].depart;
                }
                userList +='<li class="item rs-go-to-whatsapp '+active+'" data-userId="'+index+'">'+
                    '<div class="col col-1"><div class="user-pro-pic-wrapper">'+
                    '<img class="user-pro-pic" src="'+image+'" style="width:50px"></div></div><div class="col col-6">'+
                    '<div class="list-title">'+settings.users[index].name+'</div><div class="list-content">'+depart+'</div>'+
                    '<div class="list-latest-text"></div></div><div class="col col-1"><div class="list-icon-wrapper text-right">'+
            ...
            if(element !=null){
                var chatBox = '<div class="ayoan_whatsapp_chatbox_container">' +
                    ...
                    '<img class="icon" src="assets/plugins/whatsApp/img/close.svg"></button></div></div>' +
                    '<div class="rs-openChatBtn widget-close-btn-row" >' +
                    '<button type="button" class="rs-openChatBtn widget-close-btn">' +
                    '<img class="icon" src="assets/plugins/whatsApp/img/whatsapp_white.png"></button></div></div>';
                ...
                for (var i = 0; i < goToWhatsapp.length; i++) {
                    goToWhatsapp[i].addEventListener("click", function(){
                        ...
                        message = "Hola " + settings.users[parseInt(userId)].name;
...
function sendWPMessage(phone, message=''){
    ...
    apiEndPoint = apiEndPoint+'.whatsapp.com/send?phone=34' + phone + '&text=' + encodeURI(message);
